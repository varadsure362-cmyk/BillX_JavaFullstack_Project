...
import { signup, fetchCurrentUser } from '../redux/slices/authSlice';
...
export const Signup = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      fullName: Yup.string().max(150).required('Full name is required'),
      email: Yup.string().email('Invalid email').max(150).required('Email is required'),
      password: Yup.string().min(8, 'At least 8 characters').matches(/[0-9]/, 'Must contain a number').required('Password is required'),
    ...
    onSubmit: async (values) => {
      ...
      if (signup.fulfilled.match(result)) {
        ...
        setLocalError(result.payload || 'Registration failed. Email might already exist.');
      }
    },
  });
      ...
      <div className="relative z-10 mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-zinc-900/60 backdrop-blur-xl py-8 px-6 border border-zinc-700/50 rounded-2xl shadow-2xl sm:px-10">
          ...
          <form className="space-y-5" onSubmit={formik.handleSubmit} noValidate>
            ...
            <FormField label="Full Name" name="fullName" placeholder="Jane Doe" formik={formik} disabled={isBtnLoading} />
            <FormField label="Email Address" name="email" type="email" placeholder="you@example.com" formik={formik} disabled={isBtnLoading} />
            <FormField label="Password" name="password" type="password" placeholder="Min 8 chars, 1 number" formik={formik} disabled={isBtnLoading} />
            <FormField label="Phone (optional)" name="phone" type="tel" placeholder="9876543210" formik={formik} disabled={isBtnLoading} />
            ...
            >
              {isBtnLoading ? 'Creating account…' : 'Create Account'}
            </button>
          </form>
